...
from src.DataProcessing.utils.processing_vars import IRRELEVANT_WORDS_IN_ORIGIN, VARIETY_MAPPING, COLORS
...
def clean_variety_wrongly_merged(df):
    ...
    df.loc[df['variety'].str.contains('TypicaCaturraCastillo', case=False, na=False), 'variety'] = 'Typica, Caturra, Castillo'
    df.loc[df['variety'].str.contains('BourbonCaturraTypica', case=False, na=False), 'variety'] = 'Bourbon, Caturra, Typica'
    df.loc[df['variety'].str.contains('Bourbon Mundo Nuovo Robusta', case=False, na=False), 'variety'] = 'Bourbon, Mundo Nuovo, Robusta'
    df.loc[df['variety'].str.contains('Sl28Sl34Ruiru 11', case=False, na=False), 'variety'] = 'Sl28, Sl34, Ruiru 11'
...
def clean_wrong_add_ons(df):
    ...
    """
    df.loc[df['variety'].str.contains('catuai', case=False, na=False), 'variety'] = 'Catuai'
...
def clean_wrongly_annotated_kenya(df):
    ...
    """
    df.loc[df['variety'] == 'mixed Kenyan cultivars', 'variety'] = 'Sl28, Sl34, Ruiru 11, Batian'
...
def split_multiple_varieties(df):
    ...
    df_expanded = df.assign(variety=df['variety'].str.split(r'\s*[,&]\s*')).explode('variety').reset_index(drop=True)
...
def merge_rare_classes(df, threshold):
    ...
    df['origin'] = df['origin'].where(df['origin'].map(country_counts) >= threshold, 'Other')
...
def map_varieties_for_proper_merge(df, mapping_solution):
    ...
    df['variety'] = df['variety'].apply(lambda x: mapping_solution[x] if x in mapping_solution else x)
...
if __name__ == "__main__":
    ...
    coffee_love_df = get_df_without_empty_or_irrelevant_rows(original_file, IRRELEVANT_WORDS_IN_ORIGIN)
    coffee_love_origins = coffee_love_df['origin'].unique().tolist()
    coffee_love_df = coffee_love_df.apply(clean_long_titles, axis=1, origins=coffee_love_origins)

...
if analysis_type == "Overview":
    ...
    st.markdown(
        ...
        "2. In this dataset both categorical and numerical data are present. Both are handled seamlessly in a random forest.\n"
        "3. With a limited sample random forest is great due to its robustness to overfitting. For example here we have a big dominant of Brazilian coffee.\n"
    )
    ...
    st.markdown(
        ...
        "2. Due to lack of robust data about Heirloom varieties and the common labeling of Ethiopian beans as just heirloom, in this dataset we see a direct correlation of Heirloom to Ethiopian coffee. This is also largely true in real life as Ethiopia is the birthplace of coffee. Farming in this country is mostly done by gathering wildly grown beans which means it's extremely hard to label beans.\n"
        "3. Macro Average Precision and Recall show that the model is right in most cases.\n"
        "4. High F1-scores indicate that the model is good at both correctly identifying samples (recall) and making correct predictions (precision) across all coffee varieties.\n"
        "5. With all of the above we can say that the model is performing well and seems consistent across classes.\n"
        "6. he weighted averages show that the model balances predictions for both frequent and infrequent coffee varieties effectively.\n"
    )
